...
def handle_transactions():
    if request.method == 'POST':
        try:
            ...
            # --- PERBAIKAN: Handle Inventory Updates ---
            trans_type = request.form.get('type', 'Umum')
            inventory_data = []
            
            # Parse inventory data dari form
            if 'inventory_json' in request.form and request.form['inventory_json']:
                try:
                    inventory_data = json.loads(request.form['inventory_json'])
                except:
                    inventory_data = []

            # Update stok untuk pembelian
            if trans_type == 'Pembelian' and inventory_data:
                for inv_item in inventory_data:
                    product_id = inv_item.get('product_id')
                    qty = float(inv_item.get('qty', 0))
                    total = float(inv_item.get('total', 0))
                    
                    if product_id and qty > 0:
                        product = Product.query.get(product_id)
                        if product:
                            # Update moving average cost dan quantity
                            old_value = product.qty * product.avg_cost
                            new_qty = product.qty + qty
                            new_avg_cost = (old_value + total) / new_qty if new_qty > 0 else 0
                            
                            product.qty = new_qty
                            product.avg_cost = new_avg_cost
                            print(f"✅ Updated product {product.name}: qty {product.qty}, avg_cost {product.avg_cost}")

            # Update stok untuk penjualan  
            elif trans_type == 'Penjualan' and inventory_data:
                for inv_item in inventory_data:
                    product_id = inv_item.get('product_id')
                    qty = float(inv_item.get('qty', 0))
                    ...
                    if product_id and qty > 0:
                        ...
                        if product and product.qty >= qty:
                            product.qty -= qty
                            print(f"✅ Reduced product {product.name}: qty now {product.qty}")
